    ...
    methods: {
      initUserOnlineChannel() {
        ...
        userOnlineChannel.bind("pusher:subscription_succeeded", (data) => {
            // Convert members object to array and store user info
            this.onlineUsers = Object.values(data.members).map(member => member.user_info);
        });
    
        userOnlineChannel.bind("pusher:member_added", (data) => {
            const user = data.info.user_info;  // Access the nested user_info
